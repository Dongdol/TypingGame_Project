...
int Return_ErrTypeNum(char Buf[], char put_String[], int str_Size);//본문과 입력된 문자열을 비교해서 오타수 반환하는 함수
void Remove_Enter(char put_String[], int len);//gets()로 받을 때 같이 입력되는 \n 제거하는 함수
...
int main(int argc, char const *argv[])
{
 ...
 while (1)
 {
  ...
  for(int i=0; i<SIZE_READ_TEXT; i++){
	if(Buf[i] == put_String[i]){	    //if they don't match, change color
		printf("%c[32m",27);
